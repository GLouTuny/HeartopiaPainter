...
──────────────────────────
  ...
  ✓ STREAMING VERIFY with settle-time gating (time-stamped queue)
      • Each painted cell is time-stamped; verification waits for the game to render.
      • Adaptive flush: if queue grows large, verify more per paint tick.
  ✓ PER-COLOR POST-PASS VERIFY (zip-style _verify_and_repair_color_group)
      • After painting all cells in a shade, do a full verify pass (up to max_passes).
      • Re-selects the shade each pass so wrong-shade repairs actually take effect.
      • Stagnant detection: if mismatch count doesn't drop, resync UI and bail.
...
def _cell_center(canvas_rect, gw, gh, x, y):
    ...
    return (int(x0 + (x+0.5)*w/gw), int(y0 + (y+0.5)*h/gh))
...
# ── Paint coord runs ───────────────────────────────────────────────────────────
def _paint_runs(coords, c2s, opts, should_stop=None, progress_cb=None):
    coords = sorted(coords, key=lambda xy: (xy[1], xy[0])); i = 0
    while i < len(coords):
        if should_stop and should_stop(): return
        x, y = coords[i]; run = [(x,y)]; j = i+1
        while j < len(coords):
            nx, ny = coords[j]
            if ny != y or nx != run[-1][0]+1: break
            run.append((nx,ny)); j += 1
        _paint_run([c2s(rx,ry) for rx,ry in run], opts, should_stop)
        if progress_cb:
            for rx, ry in run: progress_cb(rx, ry)
        i = j
...
def _verify_and_repair_color_group(
    coords, c2s, canvas_rect, shade_rgb, group, shade, pal, opts,
    should_stop=None, status_cb=None, progress_cb=None
):
    """Full verify+repair for one color group. Re-selects shade each pass.

    Uses _sample_pixels_mss (per-pixel mss) instead of bulk ImageGrab —
    bulk ImageGrab has 1-2px coordinate rounding on small cells that causes
    false-pass results and is the primary source of the last 10-15 misses.
    Stagnant threshold raised to 3: the game can plateau for one render frame.
    First pass uses 2x settle so the game has fully rendered prior paint taps.
    """
    if not coords: return
    tol2 = opts.verify_tol ** 2
    settle = max(0.0, opts.verify_settle_s)
    first_settle = max(settle * 2, 0.08)  # longer first wait — game needs render time
    max_p = opts.verify_max_passes
    coords_sorted = sorted(coords, key=lambda xy: (xy[1], xy[0]))
    prev_miss_n: Optional[int] = None
    stagnant = 0
    stagnant_limit = max(3, opts.verify_recover_after + 1)  # was 2, now 3

    for _pass in range(max_p):
        if should_stop and should_stop(): return
        s = first_settle if _pass == 0 else settle
        if s > 0:
            if not _sleep_with_stop(s, should_stop): return
        if status_cb:
            status_cb(f"  Verify pass {_pass+1}/{max_p}: checking {len(coords_sorted)} cells…")

        # mss per-pixel — avoids bulk ImageGrab 1-2px rounding errors on small cells
        screen_pts = [c2s(px, py) for px, py in coords_sorted]
        rgbs = _sample_pixels_mss(screen_pts)
        misses = [(px, py) for (px, py), rgb in zip(coords_sorted, rgbs)
                  if rgb is not None and _d2(rgb, shade_rgb) > tol2]
        if not misses:
            if status_cb: status_cb(f"  ✓ Verified!")
            return

        miss_n = len(misses)
        if prev_miss_n is not None:
            if miss_n >= prev_miss_n: stagnant += 1
            else: stagnant = 0
        prev_miss_n = miss_n

        if stagnant >= stagnant_limit and opts.verify_auto_recover:
            if status_cb: status_cb(f"  ⚠ Verify stagnant ({miss_n} misses); resyncing UI…")
            try:
        ...
        # Re-select shade before repainting (critical for correctness)
        pal.select(group, shade, status_cb)
        if pal.cal.paint_tool_pos:
            _tap(*pal.cal.paint_tool_pos, opts)

        # Repaint misses as horizontal runs for speed
        misses.sort(key=lambda xy: (xy[1], xy[0])); k = 0
        while k < len(misses):
            if should_stop and should_stop(): return
            x, y = misses[k]; run = [(x,y)]; j = k+1
            while j < len(misses):
                nx, ny = misses[j]
                if ny != y or nx != run[-1][0]+1: break
                run.append((nx,ny)); j += 1
    ...
    if opts.verify_auto_recover:
        if status_cb: status_cb(f"  ⚠ Verify did not converge; skipping and continuing…")
        try:
            if pal.cal.shade_panel_back_xy:
                _pal_tap(*pal.cal.shade_panel_back_xy, opts)
        except Exception: pass
    ...
    for (group, shade), coords in ordered:
        ...
        if bucket_regions_ok and not disable_region_fill and len(coords) >= opts.region_min:
            ...
            for comp in comps:
                ...
                # Paint outline
                outline_opts = PaintOptions(**{**opts.__dict__, "enable_drag": True,
                ...
                _paint_runs(bnd, c2s, outline_opts, should_stop)
                time.sleep(settle)
                    ...
                    if allow_cautious or True:  # always check
                        ...
                        for bi in range(0, len(bnd), stride):
                            ...
                            for nx, ny in ((bx2-1,by2),(bx2+1,by2),(bx2,by2-1),(bx2,by2+1)):
                                if (nx,ny) not in set(map(tuple, comp)):
                                    try:
                                        cx2, cy2 = c2s(nx, ny)
        ...
        if remaining:
            # ALWAYS re-assert paint tool before pixel painting (zip-style safety)
            if pp: _tap(*pp, opts, extra_delay=0.02)
            # Re-select shade (in case UI state drifted during region fill)
            pal.select(group, shade, status_cb)

            # Streaming verify queue
            verify_queue: deque = deque()
            settle = max(0.0, opts.verify_settle_s)
            lag = opts.verify_stream_lag

            # Track whether shade is currently selected for streaming repair
            ...
            def flush_verify(force=False, max_steps=1):
                ...
                while verify_queue and (force or len(verify_queue) > lag):
                    if not force and steps >= max_steps: break
                    if should_stop and should_stop(): return
                    px, py, t_painted = verify_queue[0]
                    if settle > 0:
                        ready_in = (float(t_painted) + settle) - time.time()
                        if ready_in > 0:
                            if not force: break
                            _sleep_with_stop(min(0.02, ready_in), should_stop)
                            continue
                ...
                # Sample the batch in one mss pass
                screen_pts = [c2s(px, py) for px, py in batch]
                sampled = _sample_pixels_mss(screen_pts)
                misses = [(px, py) for (px, py), rgb in zip(batch, sampled)
                          if rgb is not None and _d2(rgb, shade_rgb) > tol2]
                if not misses: return

                # FIX: re-assert paint tool + shade before repairing streaming misses.
                # The shade panel was closed by pal.select() at the start of the color group,
                # and streaming verify happens while painting — shade IS still selected, but
                # if a repair tap runs after any palette interaction, the tool may have changed.
                if pp: _tap(*pp, opts)
                for px, py in misses:
                    if should_stop and should_stop(): return
                    cx, cy = c2s(px, py)
            ...
            _paint_runs(remaining, c2s, opts, should_stop, progress_cb=on_painted)
